...
fn main() {
    ...
    match command.as_str() {
        ...
        "sign" => {
            if args.len() < 3 {
                println!("Usage: sign <message> [--secret <hex_secret>]");
                return;
            }
            let message = &args[2];
...
fn print_usage() {
    println!("Talos Core CLI Example");
    println!("Usage:");
    println!("  cargo run --example cli -- gen-wallet [name]");
    println!("  cargo run --example cli -- sign <message>");
}
